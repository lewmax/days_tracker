...
class CustomLintRunner {
  ...
  late final initialize = Future(() async {
    ...
    await channel.sendRequest(
      PluginVersionCheckParams('', sdkPath, _analyzerPluginProtocolVersion),
    );
    await channel.sendRequest(AnalysisSetContextRootsParams(workspace.contextRoots));
  });
  ...
  Future<EditGetFixesResult> getFixes(String path, int offset) async {
    final result = await channel.sendRequest(EditGetFixesParams(path, offset));
