...
mixin LogCustomLintDelegate implements CustomLintDelegate {
  void _log(List<ContextRoot>? contextRoots, String message, {required String? pluginName}) {
    ...
    final label = pluginName != null ? '[$pluginName] ${DateTime.now().toIso8601String()}' : '';
  ...
  void pluginMessage(
    ...
    _log(pluginContextRoots, message, pluginName: pluginName);
  }
  ...
  void requestError(
    ...
    _log(allContextRoots, pluginName: null, '''
...
''');
  ...
  void pluginError(
    ...
    if (stackTrace != null) {
      _log(pluginContextRoots, '$err\n$stackTrace', pluginName: pluginName);
    } else {
      _log(pluginContextRoots, err, pluginName: pluginName);
    }
  }
}
...
class CommandCustomLintDelegate with LogCustomLintDelegate implements CustomLintDelegate {
  ...
  void serverError(
    ...
    exitCode = 1;
    super.serverError(serverPlugin, error, stackTrace, allContextRoots: allContextRoots);
  ...
  void pluginInitializationFail(
    ...
    exitCode = 1;
    super.pluginInitializationFail(serverPlugin, message, allContextRoots: allContextRoots);
  ...
  void requestError(
    ...
    exitCode = 1;
    super.requestError(serverPlugin, request, requestError, allContextRoots: allContextRoots);
    stderr.writeln('''
The request ${request.method} failed with the following error:
${requestError.code}
${requestError.message}
