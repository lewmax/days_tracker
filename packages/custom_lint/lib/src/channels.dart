...
abstract class AnalyzerPluginClientChannel {
  ...
  void sendResponse({
    ...
    sendJson(Response(requestID, requestTime, result: data?.toJson(), error: error).toJson());
  }
...
class JsonSocketChannel extends AnalyzerPluginClientChannel {
  ...
  JsonSocketChannel(this._socket) {
    ...
    _subscription = Stream.fromFuture(_socket)
        .asyncExpand((e) => e)
        .listen(_controller.add, onError: _controller.addError, onDone: _controller.close);
  }
  ...
  Iterable<List<int>> _receiveWithLength(Uint8List input) sync* {
    ...
    while (startOffset + bytesCountNeeded <= input.length) {
      if (isReadingMessageLength) {
        ...
        // We have the message length, now reading the message.
        final message = input.sublist(startOffset, startOffset + bytesCountNeeded);
        yield message;
  ...
  Future<void> sendJson(Map<String, Object?> json) async {
    ...
    _sendWithLength(socket, utf8.encode(jsonEncode(json)));
  }

  @override
  Future<void> close() async {
    await Future.wait([_subscription.cancel(), _controller.close()]);
  }
}
