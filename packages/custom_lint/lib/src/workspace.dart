...
String _buildDependencyConstraint(
  ...
  for (final (:project, :dependency) in dependencies) {
    ...
    switch ((dependency: dependency, constraint: sharedConstraint)) {
      case (:final HostedDependency dependency, :final HostedDependency? constraint):
        sharedConstraint = dependency;

        if (constraint == null) continue;

        if (constraint.hosted?.declaredName != dependency.hosted?.declaredName ||
        ...
        sharedConstraint = HostedDependency(version: newConstraint, hosted: constraint.hosted);

      case (:final PathDependency dependency, :final PathDependency? constraint):
        final absoluteDependencyPath = normalize(absolute(project.directory.path, dependency.path));
        sharedConstraint = PathDependency(absoluteDependencyPath);

        if (constraint == null) continue;
        if (constraint.path != absoluteDependencyPath) throws();

      case (:final SdkDependency dependency, :final SdkDependency? constraint):
        sharedConstraint = dependency;

        if (constraint == null) continue;
        if (constraint.sdk != dependency.sdk) throws();

      case (:final GitDependency dependency, :final GitDependency? constraint):
        sharedConstraint = dependency;
  ...
  switch (sharedConstraint) {
    ...
    case GitDependency():
      ...
      return result.toString();
    case _:
      throw StateError('Unknown constraint type: ${sharedConstraint.runtimeType}');
  }
}
...
class DependencyConstraintMeta {
  DependencyConstraintMeta._(
    ...
    required Directory workingDirectory,
       ...
       projectPath = join(
         '.',
         normalize(relative(project.directory.path, from: workingDirectory.path)),
       );
  ...
  DependencyConstraintMeta.fromDependency(
    ...
    CustomLintProject project, {
    required Directory workingDirectory,
  }) : this._(dependency.getDisplayString(), project, workingDirectory: workingDirectory);
...
class IncompatibleDependencyConstraintsException implements Exception {
  ...
  }) : assert(conflictingDependencies.length > 1, 'Must have at least 2 items');
...
Stream<YamlMap> visitAnalysisOptionAndIncludes(File analysisOptionsFile) async* {
  final visited = <String>{};
  late final packageConfigFuture =
      loadPackageConfig(
        File(join(analysisOptionsFile.parent.path, '.dart_tool/package_config.json')),
      ).then<PackageConfig?>(
        (value) => value,
...
class CustomLintWorkspace {
  ...
  static Future<CustomLintWorkspace> fromPaths(
    ...
    final foundRoots = await Future.wait(
      distinctRoots.map((rootPath) async {
        ...
        return (rootPath, optionsFile: optionFile);
      }),
    );
    ...
    return fromContextRoots(
      foundRoots.nonNulls
          .map(
            (e) => analyzer_plugin.ContextRoot(e.$1, [
  ...
  static Future<CustomLintWorkspace> fromContextRoots(
    ...
    final projects = await Future.wait([
      for (final contextRoot in contextRoots) CustomLintProject.parse(contextRoot, cache),
    ]);
  ...
  bool get isUsingFlutter =>
      projects.expand((e) => e.packageConfig.packages).any((e) => e.name == 'flutter');
...
''');
  ...
  void _writeEnvironment(StringBuffer buffer) {
    final environmentKeys = projects.expand((e) => e.pubspec.environment.keys).toSet();
      ...
      for (final name in uniqueDependencyNames)
        name: (
          dependencies: projects.expand((project) {
            ...
            return [
              if (dependency != null) (project: project, dependency: dependency),
              if (devDependency != null) (project: project, dependency: devDependency),
            ];
          }).toList(),
  ...
  void _writeDependencyOverrides(
    StringBuffer buffer, {
    required Map<String, List<({Dependency dependency, CustomLintProject project})>>
    dependencyOverrides,
  }) {
  ...
  String? computePubspecOverride() {
    ...
    _writeDependencyOverrides(buffer, dependencyOverrides: dependenciesByName);
  ...
  Future<void> resolvePluginHost(Directory tempDir) async {
...
class PubspecParseError extends Error {
  PubspecParseError._(this.path, {required this.error, required this.errorStackTrace});
...
class PackageConfigParseError extends Error {
  PackageConfigParseError._(this.path, {required this.error, required this.errorStackTrace});
...
class PluginNotFoundInPackageConfigError extends Error {
  ...
  @override
  String toString() =>
      'The plugin $name was not found in the package config '
  ...
  static Future<CustomLintProject> parse(
    ...
    final projectPubspec = await parsePubspec(projectDirectory).catchError(
      ...
      (Object err, StackTrace stack) {
        throw PubspecParseError._(directory.path, error: err, errorStackTrace: stack);
      },
    );
    ...
    final projectPackageConfig = await parsePackageConfig(projectDirectory)
        ...
        .catchError((Object err, StackTrace stack) {
          throw PackageConfigParseError._(directory.path, error: err, errorStackTrace: stack);
        });

    final plugins = await Future.wait(
      {...projectPubspec.dependencies, ...projectPubspec.devDependencies}.entries.map((e) async {
        final packageWithName = projectPackageConfig.packages.firstWhereOrNull(
          (p) => p.name == e.key,
        );
  ...
  PubspecDependency? intersect(PubspecDependency dependency) {
    ...
    return HostedPubspecDependency(
      HostedDependency(
        ...
        version: this.dependency.version.intersect(dependency.dependency.version),
      ),
    );
  }
}
