...
mixin ChannelBase {
  ...
  late final Stream<PluginErrorParams> pluginErrors = StreamGroup.mergeBroadcast([
    ...
    inputStream.where((event) => event is List).cast<List<Object?>>().map((event) {
  ...
  late final Stream<RequestError> responseErrors = responses
  ...
  Future<Response> sendRequest(RequestParams requestParams) async {
    final id = _uuid.v4();
    final request = requestParams.toRequest(id);
    final responseFuture = responses.firstWhere((message) {
      return message.id == id;
    }, orElse: () => throw StateError('No response for request ${request.method} $id'));
  ...
  /// Send a request and obtains the associated response
  Future<Response> sendRequestParams(RequestParams requestParams) async {
    final id = _uuid.v4();
...
abstract class IsolateChannelBase with ChannelBase {
  ...
  IsolateChannelBase(this.receivePort) {
    _sendPort = inputStream.where((event) => event is SendPort).cast<SendPort>().safeFirst;
  }
