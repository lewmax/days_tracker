...
class CustomLintServer {
  ...
  static Future<R> runZoned<R>(CustomLintServer Function() server, FutureOr<R> Function() body) {
    return asyncRunZonedGuarded(
      ...
      zoneSpecification: ZoneSpecification(
        print: (self, parent, zone, line) {
          unawaited(server().handlePrint(line, isClientMessage: false));
        },
      ),
    );
  }
  ...
  final _clientChannel = BehaviorSubject<SocketCustomLintServerToClientChannel?>();
  ...
  Future<void> awaitAnalysisDone({required bool reload}) => _runner.run(() async {
    final clientChannel = await _clientChannel.safeFirst;
    if (clientChannel == null) return;

    await clientChannel.sendCustomLintRequest(
      CustomLintRequest.awaitAnalysisDone(id: const Uuid().v4(), reload: reload),
    );

    // Pinging the client to flush events. This should ensure notifications are handled
    await clientChannel.sendCustomLintRequest(CustomLintRequest.ping(id: const Uuid().v4()));
  });

  Future<void> _handleRequest(Request request) async {
    final requestTime = DateTime.now().millisecondsSinceEpoch;
    Future<void> sendResponse({ResponseResult? data, RequestError? error}) async {
    ...
    try {
      final result = await request.when<FutureOr<ResponseResult?>>(
        ...
        orElse: () {
          return _runner.run(() async {
            ...
            final response = await clientChannel.sendAnalyzerPluginRequest(request);
      ...
      delegate.requestError(
        this,
        request,
        RequestError(RequestErrorCode.PLUGIN_ERROR, err.toString(), stackTrace: stack.toString()),
        allContextRoots: await _contextRoots.safeFirst.then((value) => value.roots),
      );
    }
  }
  ...
  Future<void> handleUncaughtError(Object error, StackTrace stackTrace) => _runner.run(() async {
    _analyzerPluginClientChannel.sendJson(
      PluginErrorParams(false, error.toString(), stackTrace.toString()).toNotification().toJson(),
    );

    delegate.serverError(this, error, stackTrace, allContextRoots: await _allContextRoots);
  });
  ...
  Future<void> handlePrint(String message, {required bool isClientMessage}) =>
      _runner.run(() async {
        final roots = await _contextRoots.safeFirst;

        if (!isClientMessage) {
          delegate.serverMessage(this, '$message\n', allContextRoots: roots.roots);
        } else {
          delegate.pluginMessage(this, message, pluginName: null, pluginContextRoots: roots.roots);
        }
      });
  ...
  Future<void> close() async {
    ...
    return _closeFuture =
        Future(() async {
          ...
          try {
            await Future.wait([
              _clientChannel.safeFirst.then((clientChannel) => clientChannel?.close()),
  ...
  PluginVersionCheckResult _handlePluginVersionCheck(PluginVersionCheckParams parameters) {
  ...
  Future<void> _maybeSpawnCustomLintPlugin(AnalysisSetContextRootsParams parameters) async {
    ...
    _clientChannelEventsSubscription = clientChannel.events.listen(_handleEvent);

    final configs = await Future.wait(
      parameters.roots.map((e) async {
        ...
        return CustomLintConfigs.parse(
          PhysicalResourceProvider.INSTANCE.getFile(p.join(e.root, 'analysis_options.yaml')),
          packageConfig,
        );
      }),
    );

    await clientChannel.init(debug: configs.any((e) => e != null && e.debug));
  }
    ...
    switch (event) {
      case CustomLintEventAnalyzerPluginNotification():
        ...
        if (notification.event == PLUGIN_NOTIFICATION_ERROR) {
          ...
          _analyzerPluginClientChannel.sendJson(error.toNotification().toJson());
      ...
      case CustomLintEventError():
        _analyzerPluginClientChannel.sendJson(
          PluginErrorParams(false, event.message, event.stackTrace).toNotification().toJson(),
        );
        delegate.pluginError(
          this,
          event.message,
