...
sealed class CustomLintRequest with _$CustomLintRequest {
  ...
  factory CustomLintRequest.analyzerPluginRequest(Request request, {required String id}) =
      CustomLintRequestAnalyzerPluginRequest;

  /// Requests to wait for the client to complete its analysis
  factory CustomLintRequest.awaitAnalysisDone({required String id, required bool reload}) =
...
sealed class CustomLintResponse with _$CustomLintResponse {
  ...
  factory CustomLintResponse.analyzerPluginResponse(Response response, {required String id}) =
  ...
  factory CustomLintResponse.pong({required String id}) = CustomLintResponsePong;
...
abstract class CustomLintMessage with _$CustomLintMessage {
  ...
  factory CustomLintMessage.event(CustomLintEvent event) = CustomLintMessageEvent;

  /// A response to a request
  factory CustomLintMessage.response(CustomLintResponse response) = CustomLintMessageResponse;
...
class NotificationJsonConverter extends JsonConverter<Notification, Map<String, Object?>> {
  /// A class for decoding a [Notification].
...
sealed class CustomLintEvent with _$CustomLintEvent {
  ...
  factory CustomLintEvent.error(String message, String stackTrace, {required String? pluginName}) =
      CustomLintEventError;

  /// A log output
  factory CustomLintEvent.print(String message, {required String? pluginName}) =
      CustomLintEventPrint;

  /// Decode an event from JSON
  factory CustomLintEvent.fromJson(Map<String, Object?> json) => _$CustomLintEventFromJson(json);
}
