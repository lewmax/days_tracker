...
Future<T> _asyncRetry<T>(Future<T> Function() cb, {required int retryCount}) async {
  var i = 0;
  while (true) {
    i++;
    try {
...
class SocketCustomLintServerToClientChannel {
  ...
  void _writeEntrypoint(Iterable<String> pluginNames, Directory tempDirectory) {
    ...
    final mainFile = File(join(tempDirectory.path, 'lib', 'custom_lint_client.dart'));
...
''');
  ...
  Future<CustomLintResponse> sendCustomLintRequest(CustomLintRequest request) async {
