...
class ManualRunner {
  ...
  Future<List<AnalysisErrorsParams>> getLints({required bool reload}) =>
      runner.getLints(reload: reload);

  Future<EditGetFixesResult> getFixes(String path, int offset) =>
      runner.getFixes(path, offset);

  Future<void> close() => runner.close().then((_) => channel.close());
}
